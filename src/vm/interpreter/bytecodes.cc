...
void Bytecodes::check          (Code code) { assertm(is_defined(code),      fmt::format("illegal code: %d", (int)code)); }
void Bytecodes::wide_check     (Code code) { assertm(wide_is_defined(code), fmt::format("illegal code: %d", (int)code)); }
...
void Bytecodes::def(Code code, const char* name, const char* format, const char* wide_format, BasicType result_type, int depth, bool can_trap, Code java_code) {
    assertm(wide_format == NULL || format != NULL, "short form must exist if there's a wide form");
    ...
    // todo: I don't know what's the meaning
    int bc_flags = 0;
...
//    if (java_code != code)  bc_flags |= _bc_can_rewrite;
    ...
    assertm(is_defined(code)      == (format != NULL),      "");
    assertm(wide_is_defined(code) == (wide_format != NULL), "");
    assertm(length_for(code)      == len, "");
    assertm(wide_length_for(code) == wlen, "");
}
...
int Bytecodes::compute_flags(const char* format, int more_flags) {
        ...
        case '\0':
            ...
            case 'b':
                ...
                case 'w':
                    ...
                    ++fp;  // skip 'w'
                    assertm(*fp == 'b', "wide format must start with 'wb'");
                    ++fp;  // skip 'b'
                    break;
    }
    ...
    for (;;) {
        ...
        switch (fc) {
            ...
            assertm(flags == (jchar)flags, "change _format_flags");
            return flags;
            ...
            default:  assertm(false, "bad char in format");
        }

        flags |= this_flag;

        assertm(!(has_jbo && has_nbo), "mixed byte orders in format");
        if (has_nbo)
            flags |= _fmt_has_nbo;
        ...
        if (*fp == fc) {
            ...
            switch (this_size) {
                ...
                default: assertm(false, "bad rep count in format");
            }
        }
        assertm(has_size == 0 ||                     // no field yet
...
void Bytecodes::initialize() {
    ...
    assertm(number_of_codes <= 256, "too many bytecodes");
